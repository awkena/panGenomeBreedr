...
`panGenomeBreedr` (`panGB`) is conceptualized to be a unified platform for pangenome-enabled breeding that follows standardized conventions for natural or casual variant analysis using pangenomes, marker design, and marker QC hypothesis testing (Figure 1). It seeks to simplify using pangenome resources to support plant breeding decisions during cultivar development.  
...
```{r plate_05_qc_1, fig.cap = "Fig. 2. Cluster plot for Plate 5 using FAM and HEX colors for grouping observed genotypes."}
...
kasp_qc_ggplot2(x = dat1[5],
                    ...
                    legend.pos.y = 0.75)
```
```{r plate_05_qc_2, fig.cap = "Fig. 3. Cluster plot for Plate 5 with an overlay of predictions for positive controls."}
...
library(panGenomeBreedr)
 kasp_qc_ggplot2(x = dat1[5],
                  ...
                  legend.pos.y = 0.8)
```
Color-blind-friendly color combinations are used to visualize verified genotype predictions.

In Figure 3, the three genotype classes are grouped based on plot PCH symbols using the FAM and HEX scores for observed genotype calls.

To simplify the verified prediction overlay for the expected genotypes for positive controls, all possible outcomes are divided into three categories (TRUE, FALSE, and UNKNOWN) and color-coded to make it easier to visualize verified predictions.
...
YELLOW (color code for the UNKNOWN category) means three things: an expected genotype call could not be made before KASP genotyping, or an observed genotype call could not be made to verify the prediction. 
...
```{r plate_12_design, fig.cap = "Fig. 4. Observed genotype calls for samples in Plate 12 in a plate design format."}
