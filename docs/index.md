...
\
`devtools``::`[`install_github`](https://devtools.r-lib.org/reference/install-deprecated.html)`(``"awkena/panGenomeBreedr"``)`
...
`if`` ``(``!`[`require`](https://rdrr.io/r/base/library.html)`(`[`"BiocManager"`](https://bioconductor.github.io/BiocManager/)`, quietly ``=`` ``TRUE``)``)`` `[`install.packages`](https://rdrr.io/r/utils/install.packages.html)`(``"BiocManager"``)`\
\
`# Define required Bioconductor packages`\
`bioc_packages`` ``<-`` `[`c`](https://rdrr.io/r/base/c.html)`(``"rtracklayer"``, ``"Rsamtools"``, ``"msa"``, ``"IRanges"``,`\
`                   ``"GenomicRanges"``, ``"BSgenome"``, ``"Biostrings"``, ``"GenomeInfoDb"``)`\
\
`# Install any missing Bioconductor packages`\
`for`` ``(``pkg`` ``in`` ``bioc_packages``)`` ``{`\
\
`  ``if`` ``(``!`[`requireNamespace`](https://rdrr.io/r/base/ns-load.html)`(``pkg``, quietly ``=`` ``TRUE``)``)`` ``{`\
\
`    ``BiocManager``::`[`install`](https://bioconductor.github.io/BiocManager/reference/install.html)`(``pkg``, ask ``=`` ``FALSE``, update ``=`` ``FALSE``)`\
\
`  ``}`\
`}`
...
`panGenomeBreedr``::`[`run_app`](https://awkena.github.io/panGenomeBreedr/reference/run_app.md)`(``)`
