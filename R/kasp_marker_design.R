...
kasp_marker_design <- function(vcf_file = NULL,
  ...
  if (nrow(gt_df) >= 1) {
    ...
    variant_table <- data.frame(
      variant_id = gt_df[, variant_id_col],
    ...
    geno <- data.frame(
      variant_id = variant_table$variant_id,
  ...
  variants_100bp_up <- up_down_variant(
    variant_snp_dat = variants_snp_only,
    MAF = maf,
  ...
  variants_100bp_down <- up_down_variant(
    variant_snp_dat = variants_snp_only,
    MAF = maf,
  ...
  iupac_conv <- function(iupacode, variants_100bp) {
    variants_100bp$one_letter <- iupacode$one_letter[match(
      variants_100bp$collapse,
  ...
  # upstream
  variants_100bp_up <- iupac_conv(
    iupacode = iupacode,
    variants_100bp = variants_100bp_up
  )
  # downstream
  variants_100bp_down <- iupac_conv(
    iupacode = iupacode,
  ...
  # Upstream
  upstream_sequence <- get_actual_seq(
    variants_100bp = variants_100bp_up,
    genome = genome,
  ...
  # Downdtream
  downstream_sequence <- get_actual_seq(
    variants_100bp = variants_100bp_down,
    genome = genome,
  ...
  if (save_alignment) {
    ...
    ggplot2::ggsave(
      filename = paste0("alignment_", marker_ID, ".pdf"),
      plot = pp,
      device = "pdf",
