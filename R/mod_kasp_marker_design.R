...
mod_kasp_marker_design_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    # Conditional logic
    required_pkgs <- c("openxlsx", "readxl", "shinyWidgets", "shinybusy")
    ...
    observe({
      req(vcf_data())
         updateSelectizeInput( server = TRUE, session, "marker_ID",
                          choices = vcf_data()$vcf_matrix_markerID)

         updateSelectizeInput( server = TRUE, session, "chr_ID",
    ...
    observeEvent(input$run_but, {
      ...
      if (!is.null(input$vcf_file)) {
        ...
        tryCatch({
          ...
          show_alert(
            title = "Error!",
            text = paste("Failed to design KASP from genotype data:", e$message),
            type = "error",
            showCloseButton = TRUE,
            timer = 5000
          )
