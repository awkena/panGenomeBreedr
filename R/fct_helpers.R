...
color_code_checker <- function(data){
  # Define color codes
  color_code_num <- c( 1, 0 , 0.5 , -1 , -2, -5 )
  # Respective names for color codes
  full_names <- c('Homozygous for Parent 1 allele', 'Homozygous for Parent 2 allele',
                  'Heterozygous' ,'Monomorphic loci',
                  'Genotype error loci', 'Missing parental loci')
  # Assign names to it.
  names(color_code_num) <- full_names

  result <- color_code_num[color_code_num %in% data]

  # Return also the values for
  names_not <-  c('Monomorphic loci','Genotype error loci', 'Missing parental loci')

  # Store no named vec.
  just_num <- unname(result)

  na_code <- result[which(names(result) %in% names_not)]

  return(list(color_code_in = result , na_code = na_code , just_num = just_num))
